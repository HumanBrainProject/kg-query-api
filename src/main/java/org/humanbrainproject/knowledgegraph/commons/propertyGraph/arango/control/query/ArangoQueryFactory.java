...
public class ArangoQueryFactory {
    ...
    public String querySuggestionByField(ArangoCollectionReference originalCollection, ArangoCollectionReference relationCollection, String searchTerm, Integer start, Integer size, Set<String> permissionGroupsWithReadAccess, List<ArangoCollectionReference> types) {
        ...
        if(relationCollection!=null) {
            ...
            query.add(trust("LET name = FIRST(UNION(TO_ARRAY(r.`" + SchemaOrgVocabulary.NAME + "`), TO_ARRAY(r.`http://www.w3.org/2000/01/rdf-schema#label`), TO_ARRAY(r.`http://schema.org/familyName`), TO_ARRAY(r.`https://schema.hbp.eu/minds/doi`)))"));
            if (searchTerm != null) {
                query.addLine(trust("FILTER like(r.`" + SchemaOrgVocabulary.NAME + "`, @searchTerm, true) || like(r.`http://www.w3.org/2000/01/rdf-schema#label`, @searchTerm, true) || like(r.`http://schema.org/familyName`, @searchTerm, true) || like(r.`https://schema.hbp.eu/minds/doi`, @searchTerm, true)"));
            }
            query.addLine(trust("RETURN {"));
        ...
        query.addLine(trust("LET name = FIRST(UNION(TO_ARRAY(i.`"+SchemaOrgVocabulary.NAME+"`), TO_ARRAY(i.`http://www.w3.org/2000/01/rdf-schema#label`), TO_ARRAY(i.`http://schema.org/familyName`), TO_ARRAY(i.`https://schema.hbp.eu/minds/doi`)))"));

        if (searchTerm != null) {
            query.addLine(trust("FILTER like(i.`"+SchemaOrgVocabulary.NAME+"`, @searchTerm, true) || like(i.`http://www.w3.org/2000/01/rdf-schema#label`, @searchTerm, true) || like(i.`http://schema.org/familyName`, @searchTerm, true) || like(i.`https://schema.hbp.eu/minds/doi`, @searchTerm, true)"));
        }
        query.addLine(trust("SORT i.`"+SchemaOrgVocabulary.NAME+"` ASC, i.`http://www.w3.org/2000/01/rdf-schema#label` ASC, i.`http://schema.org/familyName` ASC, i.`https://schema.hbp.eu/minds/doi` ASC"));
        query.addLine(trust("RETURN {"));
