...
//
//        Set<ArangoCollectionReference> collectionLabels= driver!=null ? driver.filterExistingCollectionLabels(edgeCollections) : edgeCollections;
//        Set<ArangoCollectionReference> collectionLabelsFiltered = collectionLabels.stream().filter( col -> !col.getName().startsWith("rel-www_w3_org")).collect(Collectors.toSet());
//        String names = String.join("`, `", collectionLabelsFiltered.stream().map(ArangoCollectionReference::getName).collect(Collectors.toSet()));
//        return String.format("" +
//                "LET doc = DOCUMENT(\"%s\")\n" +
