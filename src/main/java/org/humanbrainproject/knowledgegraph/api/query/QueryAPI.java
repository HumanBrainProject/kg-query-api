...
public class QueryAPI {
    ...
    public ResponseEntity<QueryResult> queryPropertyGraphBySpecification(@RequestBody String payload, @RequestParam(value = "vocab", required = false) String vocab, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "start", required = false) Integer start, @RequestParam(value="orgs", required = false) String organizations, @RequestParam(value="released", required = false) boolean released, @RequestParam(value="search", required = false) String searchTerm, @RequestHeader(value = "Authorization", required = false) String authorization, @ApiIgnore @RequestParam Map<String,String> allRequestParams) throws Exception {
        try {
            QueryParameters parameters = new QueryParameters().setReleased(released).setStart(start).setSize(size).setVocab(vocab).setOrganizations(organizations).setAuthorizationToken(authorization).setSearchTerm(searchTerm);
    ...
    public ResponseEntity<Map> executeQueryBasedOnTemplateWithId(@PathVariable("queryId") String queryId,@PathVariable("org") String org, @PathVariable("domain") String domain, @PathVariable("schema") String schema, @PathVariable("version") String version, @PathVariable("instanceId") String instanceId, @PathVariable("templateId") String templateId, @RequestParam(value = "size", required = false) Integer size, @RequestParam(value = "start", required = false) Integer start, @RequestParam(value="orgs", required = false) String organizations, @RequestParam(value="released", required = false) boolean released, @RequestHeader(value = "Authorization", required = false) String authorization, @ApiIgnore @RequestParam Map<String,String> allRequestParams) throws Exception {
        Template template = templating.getTemplateById(String.format("%s_%s", queryId, templateId));
        return applyFreemarkerTemplateToApiWithId(template.getTemplateContent(), queryId, org, domain, schema, version, instanceId, size, start, template.getLibrary(), organizations, released, authorization, allRequestParams);
    }
