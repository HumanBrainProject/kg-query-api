...
public class JsonLdToVerticesAndEdgesTest {
    ...
    public void transformFullyQualifiedJsonLdToVerticesAndEdgesExternal() {
        ...
        Assert.assertEquals(1, mainVertex.getEdges().size());
        Assert.assertEquals(mainVertex, mainVertex.getEdges().get(0).getFromVertex());
        Assert.assertEquals(0, mainVertex.getEdges().get(0).getProperties().size());
        Assert.assertTrue(mainVertex.getEdges().get(0) instanceof EmbeddedEdge);
        Assert.assertEquals(1, ((EmbeddedEdge)mainVertex.getEdges().get(0)).getToVertex().getProperties().size());
        Assert.assertEquals(JsonLdConsts.ID, ((EmbeddedEdge)mainVertex.getEdges().get(0)).getToVertex().getProperties().get(0).getName());
        Assert.assertEquals("http://foo.com/bar", ((EmbeddedEdge) mainVertex.getEdges().get(0)).getToVertex().getPropertyByName(JsonLdConsts.ID).getValue());
    }
